...
"""Does the v3 AICPU scheduler-metadata path survive NPUGraph capture?

`test_flash_attn_npu_v3_graph.py` captures only the forward: `get_scheduler_metadata`
...
1. capture raises -> the AICPU launch is not capturable. This is what the first run
   ...
   was replaced by a plain launch on the current stream. v3 carried the identical
   function -- line for line, only the comments differed -- and got the same fix;
...
if "Ascend910" not in _device_name:
    pytest.skip("flash_attn_with_kvcache / get_scheduler_metadata only on Ascend910", allow_module_level=True)
...
def _run(query, key_cache, value_cache, page_table, cu_seqlens_q, cache_seqlens, head_size):
    ...
    # MAX_SEQLEN_K is the page capacity, which is also what v3's
    # _validate_scheduler_metadata requires: it recomputes the expected bound as
    # k_cache.shape[1] * page_table.shape[1] and rejects a mismatch. v4 has no such
    # check, which is why the same invariant only lives in a comment there.
